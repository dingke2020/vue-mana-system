<template>
  <div>
    ...
    <el-table :data="data.data" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column label="操作" min-width="350" v-if="showOperation" header-align="center" align="left">
        <template slot-scope="scope">
          <el-button size="mini" v-if="showText(scope.$index,text)" v-show="showButtonA(scope.$index,rowA)" @click="handleLock(scope.$index, scope.row)" :style="{'color':themeColor,'border-color':themeColor}">禁用</el-button>
          <el-button size="mini" v-else v-show="showButtonA(scope.$index,rowA)" @click="handleLock(scope.$index, scope.row)" :style="{'color':themeColor,'border-color':themeColor}">启用</el-button>
          <el-button size="mini" v-show="showButtonB(scope.$index,rowB)" @click="handleEdit(scope.$index, scope.row)" :style="{'color':themeColor,'border-color':themeColor}">修改</el-button>
          <el-button size="mini" v-show="showButtonC(scope.$index,rowC)" @click="handleDelete(scope.$index, scope.row)" :style="{'color':themeColor,'border-color':themeColor}">删除</el-button>
          <el-button size="mini" v-show="showButtonD(scope.$index,rowD)" @click="handleReset(scope.$index, scope.row)" :style="{'color':themeColor,'border-color':themeColor}">重置密码</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页栏-->
...
export default {
  ...
  methods: {
    ...
    handleLock:function(index,row){
      console.log(row);
      // var title = row.isLock == 0?'禁用':'启用';
      var title = row.status == '禁用'?'启用' : '禁用';
      var isLock = row.status == '禁用'?0 : 1;
      this.$confirm('您确定'+title+'该内容吗？','确认').then(()=>{
        let params = {id:row.id,status:isLock};
        this.loading = true;
        let callback = res => {
    ...
    handleReset:function(index,row){
      console.log(row);
      this.$confirm('您确定重置密码吗？','确认').then(()=>{
        let params = {id:row.id,mobile:row.mobile};
        this.loading = true;
        let callback = res => {
          // this.$message({message: '锁定成功', type: 'success'})
          this.findPage()
        ...
        this.$emit('handle-reset', {params:params, callback:callback})
      }).catch(() => {
      })
    },
