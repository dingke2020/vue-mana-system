<template>
  ...
  <div class="personal-container">
    <ul class="personal-list">
      <li class="personal-item" v-for="(item,index) in list" :key="index" @click="showPersonal(index,item)">
        {{item.text}}
      </li>
    </ul>
  </div>
...
export default {
  ...
  data() {
    return {
      list:[{text:'个人信息'},{text:'修改密码'},{text:'退出登录'}]
    }
  },
  methods: {
    // 退出登录
    ...
    showPersonal:function(index,item){
      var arr = [index,item];
      this.$emit('show-personal',arr);
    } 
  },
  mounted() {
  }
