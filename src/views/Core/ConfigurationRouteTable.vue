<template>
  <div>
    ...
    <el-table :data="data.data" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column label="操作" min-width="150" v-if="showOperation" header-align="center" align="left">
  ...
  methods: {
    ...
    handleLock:function(index,row){
      ...
      this.$confirm('您确定'+title+'该内容吗？','确认').then(()=>{
        let params = {id:row.id,status:row.isLock};
        this.loading = true;
        let callback = res => {
