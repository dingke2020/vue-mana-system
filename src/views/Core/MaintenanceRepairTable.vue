<template>
  <div>
    ...
    <el-table :data="data.data" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column label="操作" min-width="250" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope">
          <el-button size="mini" v-if="showButtonA(scope.$index,rowA)" @click="handleComplete(scope.$index, scope.row)" :style="{'color':'#fff','border-color':themeColor,'background-color':themeColor}">完成</el-button>
          ...
          <el-button size="mini" v-if="showButtonD(scope.$index,rowD)" @click="handleCheck(scope.$index, scope.row)" :style="{'color':themeColor,'border-color':themeColor}">查看</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页栏-->
...
export default {
  ...
  mounted() {
    ...
    console.log(this.rowA,this.rowB,this.rowC,this.rowD);
  }
}
</script>
