    ...
    <el-table :data="data.data" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column label="操作" min-width="150" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope">
          <el-button size="mini" v-if="showButtonA(scope.$index,rowA)" @click="handleFill(scope.$index, scope.row)" :style="{'color':themeColor,'border-color':themeColor}">填报</el-button>
          <el-button size="mini" v-if="showButtonB(scope.$index,rowB)" @click="handleCheck(scope.$index, scope.row)" :style="{'color':themeColor,'border-color':themeColor}">查看</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页栏-->
