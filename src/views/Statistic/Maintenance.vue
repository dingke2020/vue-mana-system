<template>
  ...
  <!-- <div v-if="isTableAll" class="section-one">
    <div class="titlebar">
      <ul>
        <li><i class="iconfont icon-ali-success"></i>已保养</li>
        <li><i class="iconfont icon-ali-error"></i>未保养</li>
        <li><i class="iconfont icon-ali-stop"></i>待保养</li>
        <li><i class="iconfont icon-ali-wrench"></i>维修</li>
      </ul>
      <span>设备年度维保情况总览</span>
    ...
    <Statistic-maintenance-table :height="400" :data="pageResult" :columns="filterColumns" :customColumns="filterCustomColumns" @findPage="findPage"></Statistic-maintenance-table>
  </div> -->
  <div class="section-two">
    <div class="titlebar">
      ...
      <span>设备年度保养情况</span>
      <el-radio-group size="mini" v-model="radio1" @change="chooseRadioA">
        <el-radio-button label="图形"></el-radio-button>
        <el-radio-button label="表格"></el-radio-button>
      </el-radio-group>
    </div>
    <!--表格内容栏-->
    <div v-show="isMaintainChart"><Statistic-maintain-chart :height="tableHeight" :data="chartResultA" :columns="filterColumns" :customColumns="filterCustomColumns" @findPage="findChartA"></Statistic-maintain-chart></div>
    <div v-show="!isMaintainChart"><Statistic-maintain-table :height="tableHeight" :data="pageResultA" :columns="filterColumnsA" @findPage="findPageA" :rowstatus="tableRowClassNameA"></Statistic-maintain-table></div>
  </div>
  <!-- <div v-if="isTableB" class="section-three">
    <div class="titlebar">
      <span>设备年度维修情况</span>
    ...
    <div v-show="isRepairChart"><Statistic-repair-chart :height="400" :data="pageResult" :columns="filterColumnsCopy" :customColumns="filterCustomColumns" @findPage="findPage"></Statistic-repair-chart></div>
    <div v-show="!isRepairChart"><Statistic-repair-table :height="400" :data="pageResultB" :columns="filterColumnsB" @findPage="findPageB" :rowstatus="tableRowClassNameB"></Statistic-repair-table></div>
  </div> -->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    findPageA: function (data) {
      ...
      //这里是不调接口的
      _this.$api.statistic.findPageAMaintenance(_this.pageRequest).then((res) => {
        console.log(res);
        var newArray = res.data.data.maintenance_a_list.data;
        ...
        _this.pageResultA = res.data.data.maintenance_a_list;
        console.log(_this.pageResultA)
        // this.findUserRoles()
      ...
      _this.$api.statistic.findPageAMaintenance(_this.pageRequest).then((res) => {
        ...
        _this.pageResultA = res.data;
        console.log(_this.pageResultA)
        // this.findUserRoles()
      }).then(data!=null?data.callback:'')
    },
    findChartA: function (data) {
      console.log(data);
      let _this = this;
      if(data !== null) {
        data.pageRequest.year = _this.year;
      ...
      //这里是不调接口的
      _this.$api.statistic.findChartAMaintenance(_this.pageRequest).then((res) => {
        console.log(res);
        var newArray = res.data.data.maintenance_a_chart.data;
        ...
        newArray.forEach(function(value,index){
          ...
          value.january = [{"state":value.january}];
          value.february = [{"state":value.february}];
          value.march = [{"state":value.march}];
          value.april = [{"state":value.april}];
          value.may = [{"state":value.may}];
          value.june = [{"state":value.june}];
          value.july = [{"state":value.july}];
          value.august = [{"state":value.august}];
          value.september = [{"state":value.september}];
          value.october = [{"state":value.october}];
          value.november = [{"state":value.november}];
          value.december = [{"state":value.december}];
        })
        console.log(newArray);
        _this.chartResultA = res.data.data.maintenance_a_chart;
        console.log(_this.chartResultA)
        // this.findUserRoles()
      }).then(data!=null?data.callback:'')
      _this.$api.statistic.findChartAMaintenance(_this.pageRequest).then((res) => {
        console.log(res);
    ...
    searchPage: function () {
      ...
      if(_this.isMaintainChart){
        _this.pageParams = {year:_this.year,machineType:_this.machine,nameCode: _this.dataForm.device};
        _this.$api.statistic.findChartAMaintenance(_this.pageParams).then((res) => {
          console.log(res);
          var newArray = res.data;
          console.log(newArray);
          newArray.forEach(function(value,index){
        ...
        _this.$api.statistic.findPageAMaintenance(_this.pageParams).then((res) => {
          console.log(res);
          var newArray = res.data.data;
          newArray.forEach(function(value,index){
            switch(value.status){
...
    },
    initColumnsA: function () {
      this.columnsA = [
        // {prop:"id", label:"序号", minWidth:50},
        {prop:"month", label:"年月份", minWidth:90},
        ...
        {prop:"maintenanceDate", label:"保养时间", minWidth:110},
        {prop:"ddPerson", label:"担当人", minWidth:90},
        {prop:"zrPerson", label:"责任人", minWidth:90},
        {prop:"remark", label:"备注", minWidth:90},
        {prop:"updateTime", label:"填报时间", minWidth:110},
        {prop:"status", label:"状态", minWidth:100}
      ]
      this.filterColumnsA = JSON.parse(JSON.stringify(this.columnsA));
    },
//     initColumnsB: function () {
//       this.columnsB = [
//         // {prop:"id", label:"序号", minWidth:50},
//         {prop:"equipmentCode", label:"设备编号", minWidth:100},
//         {prop:"equipmentName", label:"设备名称", minWidth:100},
//         {prop:"equipmentModel", label:"设备型号", minWidth:100},
//         {prop:"faultCause", label:"故障原因", minWidth:100},
//         {prop:"faultTime", label:"故障时间", minWidth:110},
//         {prop:"completionTime", label:"完成时间", minWidth:110},
//         {prop:"repair", label:"维修人", minWidth:90},
//         {prop:"acceptor", label:"验收人", minWidth:90},
//         {prop:"repairStatus", label:"维修状态", minWidth:100}
//       ]
...
//       this.columnsCopy = [
//         // {prop:"id", label:"序号", minWidth:50},
//         {prop:"equipmentName", label:"设备名称", minWidth:90},
//         {prop:"equipmentCode", label:"设备编号", minWidth:90},
...
//     },
    ...
    tableRowClassNameA({row, rowIndex}){
      var length = $(".maintain .el-table__body-wrapper tbody tr:eq(0) td").length - 1;
      if(row.status == "未保养"){
        $(".maintain .el-table__body-wrapper tbody tr:eq("+rowIndex+") td:eq("+(length)+")").css({"color":"#FF9800"});
      }else if(row.status == "已保养"){
        $(".maintain .el-table__body-wrapper tbody tr:eq("+rowIndex+") td:eq("+(length)+")").css({"color":"#3F51B5"});
      }
    },
    // tableRowClassNameB({row, rowIndex}){
    //   var length = $(".repair .el-table__body-wrapper tbody tr:eq(0) td").length - 1;
    //   if(row.repairStatus == "待检修"){
    //     $(".repair .el-table__body-wrapper tbody tr:eq("+rowIndex+") td:eq("+(length)+")").css({"color":"#FF9800"});
    //   }else if(row.repairStatus == "已完成"){
