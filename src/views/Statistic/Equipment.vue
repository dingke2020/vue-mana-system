<template>
  ...
  <div v-if="!isTable" :style="{height:tableHeight+'px'}" style="position:relative;">
    <div class="chart-title chart-title-left">生产时长</div>
     <div class="chart-title chart-title-right">开机时长</div>
    <div id="job" style="width:100%;height:100%"></div>
  </div>
  <!--表格内容栏-->
  <div v-if="isTable"><Statistic-equipment-table :height="tableHeight" :data="pageResult" :columns="filterColumns" @findPage="findPage"></Statistic-equipment-table></div>
  </div>
</template>

<script>
import { mapState } from 'vuex'
import StatisticEquipmentTable from "@/views/Core/StatisticEquipmentTable"
import { format } from "@/utils/datetime"
export default {
  components:{
    StatisticEquipmentTable
  },
  data() {
    return {
      inputs:[{}],
      ...
      radio1:'图形',
      mychartOne:'',
      xAxisDate:['6/1','6/2','6/3','6/4','6/5','6/6','6/7','6/8','6/9','6/10','6/11','6/12','6/13','6/14','6/15','6/16','6/17','6/18','6/19','6/20','6/21','6/22','6/23','6/24','6/25','6/26','6/27','6/28','6/29','6/30'],
      productionTime:[4.2,4.2,3.2,3.4,4.2,2.8,1.8,1.8,1.8,1.6,2.8,1,2.8,4.5,4.5,5.2,5.2,3.2,3.2,4.3,1.9,1.9,2.9,2.9,1,4.3,1.9,3.1,3.1,4.2],
      startTime:[5.8,5.8,4.8,5.2,5.8,3.5,2.5,2.5,2.5,1.8,3.7,1.2,3.7,7.2,7.2,7.8,7.8,4.2,4.2,7.1,3.1,3.1,3.9,3.9,1.1,7.1,2.9,4.5,4.5,5.8],
      stackTime:[],
      dateArr:[],
      deviceAll:[],
      deviceForm:{
  ...
  methods: {
    ...
    findPage: function (data) {
      ...
      //这里是不调接口的
      _this.$api.statistic.findPageEquipment(_this.pageRequest).then((res) => {
        console.log(res);
        var newArray = res.data.data.equipment_list.data;
        newArray.forEach(function(value,index){
          
        })
        _this.pageResult = res.data.data.equipment_list;
        console.log(_this.pageResult)
        // this.findUserRoles()
      }).then(data!=null?data.callback:'')
      $.get('/xaj/statistic/searchEquipmentPerformanceList',_this.pageRequest).then((res) => {
        console.log(res);
    ...
    searchPage: function () {
      ...
      _this.pageParams = {currPage: 1, pageSize: 10,startTime: start,endTime: end,period: _this.select,nameCode: _this.dataForm.device};
      _this.pageRequest.pageSize = 10;
      _this.pageRequest.currPage = 1;
      $.get('/xaj/statistic/searchEquipmentPerformanceList',_this.pageParams).then((res) => {
        console.log(res);
        var newArray = res.data;
        newArray.forEach(function(value,index){
...
    initColumns: function () {
      this.columns = [
        ...
        {prop:"machine", label:"设备名称", minWidth:80},
        {prop:"machineNumber", label:"设备编号", minWidth:80},
        {prop:"groupName", label:"所属车间", minWidth:80},
        ...
        {prop:"productionProcessesName", label:"生产工序", minWidth:80},
        {prop:"name", label:"生产人员", minWidth:80},
        {prop:"nowQuantity", label:"生产数量", minWidth:80},
    ...
    querySearch(queryString,cb){
      ...
      findChart:function(){
        ...
        _this.chartRequest = {
          ...
          startTime: start,
          endTime: end,
          nameCode: _this.dataForm.device
        }
        _this.xAxisDate = [];
        _this.startTime = [];
        ...
        _this.stackTime = [];
        //这里是不调接口的
        _this.$api.statistic.findChartEquipment(_this.chartRequest).then((res) => {
          console.log(res);
          var data = res.data.data.equipmemt_chart.data;
          $.each(data,function(index,value){
            ...
            _this.startTime.push(value.workingTime);
            _this.productionTime.push(value.productionTime);
            _this.stackTime.push(((value.workingTime*100-value.productionTime*100)/100).toFixed(1));
            console.log(_this.stackTime);
            console.log(_this.xAxisDate,_this.startTime,_this.productionTime);
          })
          ...
          setTimeout(function(){
            _this.mychartOne.setOption({
              grid:{
                ...
                left:7*_this.chart_fontSize_10,
              ...
              legend:{
                ...
                selectedMode:false,
                data:[{name:'生产时长(hour)',icon:'rect'},{name:'开机时长(hour)',icon:'rect'}]
              },
              xAxis:{
                axisLine:{
                  lineStyle:{
              ...
              series:[{
                ...
                label:{
                  show:true,
                  color:'#FFF',
                  fontSize:_this.chart_fontSize_10,
                  position:'insideTop',
                  fontFamily:'SourceHanSansSC-regular'
                },
                itemStyle:{
                ...
                name:'开机时长(hour)',
                yAxisIndex:0,
                barMinHeight:2*_this.chart_fontSize_10,
                stack:'时长',
                barWidth:'80%',
                label:{
                  show:true,
                  ...
                  formatter:function(params){
                    return _this.startTime[params.dataIndex];
                  }
                },
                itemStyle:{
                  color:'#485ABF',
                ...
                data:_this.stackTime
              }]
            })
          },500);
        }).then()
        $.get('/xaj/statistic/searchEquipmentPerformanceChart',_this.chartRequest).then((res) => {
          console.log(res);
          var data = res;
          $.each(data,function(index,value){
            if(_this.select == "byYear"){
      ...
      $.ajax({
          ...
          success:function(data){
            ...
            $.each(newArr,function(index,value){
              ...
              if(flag){
                nameArr.push({value:newArr[index].machine});
              }
            })
            var numberArr = [];
            $.each(newArr,function(index,value){
